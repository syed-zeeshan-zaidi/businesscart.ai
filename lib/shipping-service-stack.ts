...
dotenv.config({ path: join(__dirname, '..', 'shipping-service', '.env') });

export class ShippingServiceStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    this.handler = new lambda.Function(this, 'ShippingHandler', {
      runtime: lambda.Runtime.NODEJS_18_X,
      handler: 'handler.handler',
      code: lambda.Code.fromAsset(join(__dirname, '..', 'shipping-service', 'dist')),
      environment: {
    ...
    this.api = new apigw.RestApi(this, 'ShippingApi', {
      restApiName: 'ShippingService',
      deployOptions: {
        stageName: 'dev',
      },
    });

    const shippingResource = this.api.root.addResource('shipping');
    shippingResource.addMethod('POST', new apigw.LambdaIntegration(this.handler));
  }
}